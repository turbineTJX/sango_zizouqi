...
export const TACTICS_BOOK = {
  ...
  wildfire: scheme('火计',80,30,'wildfire','fire','对相邻最多 2 队造成 0.85 倍谋略伤害并灼烧 6 步；主目标已燃烧时命中伤害提高至 1.25 倍；灼烧每层为当前谋略威力×(6/280＋0.04)，受军纪抵御；燃击 12 步，普攻续叠灼烧（最多 3 层、刷新 6 步）；自身谋攻 16 步，普攻改用 0.8 倍谋略威力、受军纪抵御',4),
  ...
  fire: tactic('火矢',40,22,'fire','fire','射击（1.0 倍武技）并灼烧 6 步；自身燃击 10 步，普攻续叠灼烧，最多 3 层、刷新 6 步；每层为当前武技威力×(6/280＋0.03)，受军纪抵御；新燃击按当时面板威力计算'),
...
export function setStatus(b,u,key,duration,extra={}) {
  ...
  if(key==='curse'){const current=hasStatus(b,u,'curse')?u.statuses.curse:null;const stacks=Math.min(3,(current?.stacks||0)+1);u.statuses.curse={until:b.tick+duration+1,stacks,potency:Math.max(current?.potency??0,extra.potency??1)};return;}
