...
import {famousTargets, expandedSupportTargets, basicSupportTargets, areaTacticTargets, exposedTarget, tauntTarget, pursuitTarget, flankingTarget, supportAnchor, repairTarget} from './tactics.mjs';
...
export function validateSave(value, { strategic = false } = {}) {
  ...
  for (const army of value.armies) {
    ...
    armyIds.add(army.id);
    require(Array.isArray(army.units) && army.units.length > 0 && army.units.length <= (strategic ? Object.keys(OFFICER_BY_ID).length : scenario ? 30 : 15) && Array.isArray(army.route) && army.route.length <= 9 && army.route.every(id => cityById(value, id)) && number(army.supply) && number(army.morale, 100), '军团数据不完整');
