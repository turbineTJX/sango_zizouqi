...
test('cavalry offensive stance increases basic attack at the cost of real defense',()=>{
  ...
  assert.equal(after.attack,before.attack*(1+.25*powerFactor(before.martialPower)));assert.equal(after.defense,before.defense*.8);
...
test('screen protects threatened healthy allies, respects its own layer and caps at one recipient',()=>{
  ...
  assert.equal(shieldAmount(x.b,protectedUnits[0]),Math.round(protectedUnits[0].maxHp*.04*powerFactor(unitAttributes(x.a,x.b).strategyPower)));
