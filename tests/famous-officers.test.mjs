...
test('multi-hit and multi-target exclusive attacks generate intent only once per cast',()=>{
 for(const id of ['person-390','person-603']){
  ...
  stepBattle(b);assert.equal(u.intent,Math.min(100,s.threshold+12));
 }
});
...
test('support AI avoids idle shields, prioritizes cleansing, and skips a useless high-priority slot',()=>{
 ...
 q.u.type='crossbow';q.u.tactics=[q.s.id,'seal','harry'];q.u.skillReady={};
...
test('weaker burns do not erase stronger snapshots or prolong them; stronger burns replace attribution',()=>{
 const {b,d}=fixture('person-246');
 setStatus(b,d,'burn',3,{amount:40,sourceId:'strong'});const before=structuredClone(d.statuses.burn);
 setStatus(b,d,'burn',8,{amount:15,sourceId:'weak'});assert.deepEqual(d.statuses.burn,before);
 setStatus(b,d,'burn',5,{amount:45,sourceId:'stronger'});assert.equal(d.statuses.burn.sourceId,'stronger');
...
test('self-cost bypasses shields, preserves one survivor and updates the casualty ledger without extra intent',()=>{
 for(const id of ['person-164','person-494']){
  ...
  assert.equal(shieldAmount(b,u),500);assert.equal(u.intent,s.threshold+12);
