...
test('combo enhances damage-over-time, intent reduction and control duration while respecting immunity',()=>{
 ...
 const y=scene();y.own[1].x=4;cast(y,y.own[0],'repeat');y.target.intent=100;cast(y,y.own[1],'harass');assert.equal(y.target.intent,100-Math.round((12+Math.round(unitAttributes(y.own[1],y.b).strategyPower*.1))*1.25));
