...
test('ordinary aptitude routes stay fixed through troop changes and keep specialization tradeoffs',()=>{
 for(const [id,key] of [['person-69','spear'],['person-559','cavalry'],['person-46','archer'],['person-447','strategist'],['person-567','support']]){
 ...
 assert.match(rosterMarkup({query:'满宠'}),/辅军修习/);assert.doesNotMatch(rosterMarkup({query:'满宠'}),/技能待设计/);
});
...
test('ordinary officers use legal support and damage builds in real combat and resume deterministically',()=>{
 ...
 for(const units of [state.armies[0].units,b.sides[0].units])units.forEach((u,i)=>{u.type=configs[i][0];assert.equal(configureTactics(u,configs[i][1]),null);});
 lockDeployment(b);for(let i=0;i<12;i++)stepBattle(b);const resumed=validateSave(structuredClone(state));
