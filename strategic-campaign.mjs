...
export const CAMPAIGN = Object.freeze({version:1, daysPerTurn:10, stepsPerDay:12, maxUnits:10, maxArmies:15, supplyRange:180});
...
export function newCampaign(seed=521200){
  ...
  s.logs=[];log(s,'旬回合筹划，逐日执行。军团途中接敌，战役每日快照可随时接管。','event');syncResources(s);return s;
}
...
export function orderCampaignArmy(s,id,target){
  ...
  a.route=a.travel?[a.travel.to,...route]:route;a.target=a.route.length?target:null;a.task=a.route.length?'行军':'驻守';return null;
}
export function splitCampaignArmy(s,id,ids){
  ...
  if(!chosen.length||chosen.length===a.units.length||chosen.length>10||!chosen.some(u=>u.troops)||!a.units.some(u=>!ids.includes(u.id)&&u.troops))return '两支军团都需保留有兵力的部队';
  const ratio=chosen.length/a.units.length,capacity=Math.floor(a.supplyCapacity*ratio),supply=round(a.supply*ratio);
  const b={...copy(a),id:`a${s.nextId++}`,name:`${chosen[0].name}军`,units:chosen,leader:chosen[0].id,advisor:[...chosen].sort((x,y)=>y.intellect-x.intellect)[0].id,deputy:chosen[1]?.id||null,supplyCapacity:capacity,supply,route:[],target:null};
...
function normalize(a){for(const key of ['leader','advisor','deputy'])if(!a.units.some(u=>u.id===a[key]))a[key]=key==='deputy'?null:a.units[0].id;if(!a.units.some(u=>u.first))a.units.slice(0,6).forEach(u=>u.first=true);}
export function mergeCampaignArmies(s,into,from){
  ...
  if(a.units.length+b.units.length>10)return '军团最多编制 10 支部队';
  b.units.forEach(u=>u.first=false);a.units.push(...b.units);a.supply=round(a.supply+b.supply);a.supplyCapacity+=b.supplyCapacity;a.hunger=Math.max(a.hunger,b.hunger);s.armies=s.armies.filter(x=>x!==b);return null;
}
export function createCampaignArmy(s,cityId,ids){
  ...
  if(!officers.length||officers.length>10||s.armies.length>=15)return '请选择 1～10 位在城武将';
  if(officers.some(o=>s.cities.some(x=>x.governor===o.unit.id)))return '请先解除太守任命';
  const units=officers.map((o,i)=>({...o.unit,first:i<6}));
...
export function transferOfficer(s,id,destination){
  ...
  if(!distance)return null;o.destination=destination;o.remainingDays=Math.ceil(distance/35);o.unit.homeCity=destination;return null;
}
export function recruitCampaign(s,id){
  const a=army(s,id);if(!canEditArmy(s,a)||a.faction!=='cao')return '只能在筹划阶段于友城整补';
  const c=city(s,a.location),need=a.units.reduce((n,u)=>n+Math.max(0,troopCapacity(u)-u.troops-u.wounded),0);
  const amount=Math.floor(Math.min(need,c.manpower,2000+c.barracks*1000-c.drafted,s.gold*4,c.grain));
  if(amount<=0)return '兵员已足，或本旬征募额度、府库、兵源、粮草不足';
  let left=amount;for(const u of [...a.units].sort((a,b)=>a.troops-b.troops)){const n=Math.min(left,troopCapacity(u)-u.troops-u.wounded);u.troops+=n;left-=n;}
...
export function commissionProject(s,cityId,key){
  ...
  if(c.project)return '该城已有建设任务';if(c[p.field]>=5)return '设施已达五级';if(s.gold<p.cost)return '府库不足';
  if(activeBattles(s).some(r=>r.kind==='siege'&&r.cityId===cityId))return '围城期间无法开工';
  s.gold-=p.cost;c.project={key,remaining:p.turns};return null;
}
export function relieveCity(s,cityId){
  const c=city(s,cityId);if(!isPlanning(s)||c?.owner!=='cao')return '只能在筹划阶段赈济';
  if(c.reliefTurn===s.turn)return '本旬已经赈济';if(c.order>=100)return '民心已满';if(s.gold<150||c.grain<300)return '赈济需要 150 金、300 粮';
  s.gold-=150;c.grain-=300;c.order=Math.min(100,c.order+15);c.reliefTurn=s.turn;syncResources(s);return null;
}
...
export function cityIncome(s,c){
  const governor=s.campaign.idle.find(o=>o.unit.id===c.governor&&!o.destination&&o.location===c.id);
  const factor=(.5+c.order/200)*(1+(governor?.unit.politics||0)/500);
  return {gold:Math.floor((160+c.commerce*160)*factor),grain:Math.floor((600+c.farm*600)*factor),manpower:Math.floor((500+c.barracks*500)*factor)};
}
function finishTurn(s){
  const summary=[];
  for(const c of s.cities){
    const besieged=activeBattles(s).some(r=>r.kind==='siege'&&r.cityId===c.id);
    if(!besieged&&c.project){c.project.remaining--;if(c.project.remaining<=0){const p=PROJECTS[c.project.key];c[p.field]++;if(p.field==='walls')c.gateHp+=3000;summary.push(`${c.name}完成${p.name}`);c.project=null;}}
    c.drafted=0;if(besieged)continue;
    const income=cityIncome(s,c);if(c.owner==='cao')s.gold+=income.gold;
    ...
    if(c.owner==='cao')summary.push(`${c.name}：金 +${income.gold}，粮 +${income.grain}，兵源 +${income.manpower}`);
  }
...
function dailySupply(s){
  ...
  for(const a of [...s.armies].filter(a=>!a.disbanded).sort((a,b)=>a.supply/Math.max(1,dailyConsumption(s,a))-b.supply/Math.max(1,dailyConsumption(s,b))||a.id.localeCompare(b.id))){
    ...
    if(!r&&!a.travel&&!a.route.length&&city(s,a.location).owner===a.faction&&a.hunger===0){for(const u of a.units){const n=Math.min(u.wounded,18+city(s,a.location).barracks*6);u.wounded-=n;u.troops+=n;}}
  }
  syncResources(s);
}
function enemyOrders(s){
  for(const a of s.armies.filter(a=>a.faction==='yuan'&&!a.disbanded)){
    if(a.route.length||armyBattle(s,a.id)||!armyTroops(a))continue;
    const targets=s.cities.filter(c=>c.owner==='cao').map(c=>({c,route:findCampaignRoute(s,a.travel?.to||a.location,c.id)})).filter(x=>x.route);
...
export function beginExecution(s){
  ...
  enemyOrders(s);s.campaign.phase='executing';
...
function terrainFor(s,from,to){if([from,to].includes('baima'))return 'river';if([from,to].includes('jinyang'))return 'hill';if([from,to].includes('wan'))return 'forest';if([from,to].includes('runan'))return 'marsh';return 'land';}
function makeEncounter(s,attacker,defenders,cityId,kind,point){
  ...
  const proxy=newGame(s.seed+s.nextId*997);proxy.relationshipScores=copy(s.relationshipScores);proxy.relationshipTypes=copy(s.relationshipTypes);
  ...
  if(kind==='siege')for(const u of b.sides[1-attackSide].units.filter(u=>u.status==='active'))setStatus(b,u,'shield',1000,{amount:Math.round(u.initial*.15),source:'siege:opening',label:'守城首发护盾'});
...
function moveArmies(s){
  ...
  for(const m of movements){const a=m.a;if(armyBattle(s,a.id))continue;
    ...
    if(c.owner!==a.faction){c.owner=a.faction;c.order=40;c.project=null;c.governor=null;}
...
function autoCommand(b){
  ...
  const view={...b,sides:[b.sides[1],b.sides[0]].map((side,index)=>({...side,units:side.units.map(u=>({...u,side:index}))})),enemyCommand:b};
  const key=chooseEnemyCommand(view,battleStratagems(b,0),STRATAGEMS);if(key)issueCommand(b,key);
}
...
function settleEncounter(s,r){
  ...
  if(r.kind==='siege'){
    ...
    if(b.result.winner===r.attackSide){c.owner=b.sides[r.attackSide].faction;c.garrison=0;c.order=Math.min(c.order,40);c.governor=null;c.project=null;}
  }
...
function prepareDay(s){
  ...
  dailySupply(s);moveArmies(s);syncResources(s);c.dayPrepared=true;
...
export function advanceCampaignStep(s){
  ...
  const boundary=c.day%10===0;if(boundary)finishTurn(s);
...
export function validateCampaign(value){
  ...
  const c=value?.campaign;fail(c?.version===CAMPAIGN.version,'战略存档版本不兼容，请重新开始');
  ...
  fail(typeof c.lastNotice==='string'&&c.lastNotice.length<1000&&c.turnReports.every(r=>integer(r.turn)&&Array.isArray(r.items)&&r.items.length<=30&&r.items.every(x=>typeof x==='string'&&x.length<1000)));
  // Core officer, map and combat validation remains shared with the real engine.
  const envelope={...value,battle:null,pending:null,report:null,armies:value.armies?.map(a=>({...a,supply:Math.ceil(a.supply)}))};validateSave(envelope);
  ...
  for(const a of value.armies){fail(a.units.length<=10&&a.units.filter(u=>u.first).length<=6&&num(a.supply)&&integer(a.supplyCapacity)&&a.supply<=a.supplyCapacity&&num(a.hunger,5)&&num(a.supplyIn)&&integer(a.cooldownDay)&&city(value,a.homeCity),'军团编制或粮草无效');a.units.forEach(checkOfficer);
  ...
  for(const o of c.idle){checkOfficer(o.unit);fail(['cao','yuan','neutral'].includes(o.faction)&&city(value,o.location)&&integer(o.remainingDays)&&(!o.destination?o.remainingDays===0:city(value,o.destination)&&o.remainingDays>0),'武将调任状态无效');
  ...
  for(const town of value.cities){for(const key of ['grain','manpower','order','gateHp'])fail(num(town[key]),'城池资源无效');fail(town.order<=100&&town.grain<=10000+town.granary*10000&&town.gateHp<=12000+town.walls*3000);
    ...
    fail(!town.project||Object.hasOwn(PROJECTS,town.project.key)&&integer(town.project.remaining,PROJECTS[town.project.key].turns)&&town.project.remaining>0,'建设队列无效');
  ...
  for(const r of c.battles){fail(r&&typeof r.id==='string'&&!battleIds.has(r.id)&&typeof r.name==='string'&&r.name.length<100&&['field','siege'].includes(r.kind)&&city(value,r.cityId)&&num(r.point?.x,1000)&&num(r.point?.y,800));battleIds.add(r.id);
    ...
    if(r.settled)fail(r.report&&r.report.winner===r.battle.result.winner&&r.report.reason===r.battle.result.reason&&Array.isArray(r.report.stats)&&r.report.stats.length===2&&r.report.stats.every(x=>['cao','yuan','neutral'].includes(x.faction)&&['initial','remaining','wounded','killed','escaped'].every(k=>integer(x[k]))&&x.initial===x.remaining+x.wounded+x.killed+x.escaped)&&Array.isArray(r.report.growth),'战果记录无效');
  ...
  value.battle=c.focusId?battleRecord(value,c.focusId).battle:null;value.pending=null;value.report=null;return value;
}
