...
for(const c of TACTICAL_CAMPAIGNS.filter(c=>c.id.startsWith('tactical-control'))){
 ...
 try{
  c.ownTeam.forEach(u=>{if(u.type==='archer')u.tactics=['fire','scatter','suppress'];if(u.type==='crossbow')u.tactics=['pierce','repeat','retreatShot'];});
  const r=sample(c.id,count,start);comparisons.push({name:c.name,variant:'前排不变，谋士改成物理输出配装',...r});console.log('纯输出',c.name,r.wins,r.losses,r.draws);
