...
for(let i=0;i<4;i++)add(`equipped${i}`,`1级已学槽${i+1}`,18,'战法',x=>unitTactics(x.u)[i]?.name||'未学会');
...
add('personalityCode','性格编号',12,'人物',x=>x.c.personality,'0');
add('righteousnessCode','义理编号',12,'人物',x=>x.c.righteousness,'0');
...
for(const [key,label] of [['spearLv','枪兵'],['halberdLv','戟兵'],['crossbowLv','弩兵'],['rideLv','骑兵'],['machineLv','兵器'],['waterLv','水军']])add(key,`来源${label}适性`,15,'适性',x=>['C','B','A','S'][x.c.profileSource[key]]??'未载');
