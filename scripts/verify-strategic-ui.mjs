...
try{
  ...
  for(let i=0;i<24&&(await saved()).campaign.day<11;i++){if(await action('campaign-auto').count()){await action('campaign-auto').first().click();await pauseMap();}else await day();}
  ...
  await action('campaign-begin').click();
  if(!paused.campaign.battles.find(b=>b.id===r.id).settled){await page.locator('#battle-board').waitFor();await action('pause').click();assert.equal((await saved()).campaign.focusId,r.id);await action('campaign-map').click();}
  await page.locator('.strategy-clock').waitFor();await pauseMap();await page.locator('[data-action="campaign-tab"][data-tab="battles"]').click();
  ...
  await action('lobby').click();await page.locator('.campaign-lobby').waitFor();await action('campaign-lobby').click();await action('launch-history').click();await page.locator('#battle-board').waitFor();
