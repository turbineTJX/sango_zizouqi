...
const SAVE_KEY = 'sango-sovereign-v1';
...
function unitStatusMarkup(u,b) {
  ...
  const shield=hasStatus(b,u,'shield') ? u.statuses.shield.amount : 0;
...
function renderModal() {
  ...
  if (ui.modal === 'officers') html = modalShell('麾下群英', 'OFFICERS · 武将名录', `<div class="officer-grid">${state.armies.filter(a => a.faction === 'cao').flatMap(a => a.units.map(u => `<article class="officer-card">${avatar(u)}<div><h3>${u.name}<small>字 ${u.courtesy}</small></h3><span class="trait">${u.trait}</span></div><p>${unitTactics(u).map(s=>s.name).join(' · ')}<span>${TROOPS[u.type].name}</span></p><div class="officer-stats">${[['统率', u.leadership], ['武力', u.force], ['智力', u.intellect], ['政治', u.politics], ['魅力', u.charm]].map(([n, v]) => `<div><b>${v}</b><small>${n}</small></div>`).join('')}</div><p class="officer-strategies">拥有军略：${officerStratagems(u.id).map(key=>STRATAGEMS[key].name).join(' · ') || '无'}<small>任主将或军师时解锁</small></p><footer>${esc(a.name)} · ${cityById(state, a.location).name}<span>忠诚 ${u.loyalty}</span></footer></article>`)).join('')}</div><p class="muted">当前原型实现战斗属性与编制；人物关系、招募和成长留待后续版本。</p>`, '', true);
...
document.addEventListener('keydown', event => {
  ...
  if (event.key === 'Tab' && ui.modal) {
    const nodes = [...document.querySelectorAll('.modal button:not([disabled]), .modal input:not([hidden]):not([disabled]), .modal select:not([disabled])')];
