...
export const TROOPS = {
 ...
 logistics:{name:'辅兵',icon:'辅',attack:30,defense:55,discipline:45,move:1,interval:4,range:1,siege:0,beats:null},
...
export const isRear=u=>['archer','crossbow','siege'].includes(u.type);
...
export function unitAttributes(u,b=null) {
 ...
 const troopAttributes=new Set(['attack','martialPower','strategyPower','supportPower','siege']);
 ...
 if(army('assaultUntil'))atk.push({label:'全军猛攻',factor:1.25});
 if(army('fortifyUntil')){def.push({label:'坚壁之策',factor:1.2});discipline.push({label:'坚壁之策',factor:1.2});}
 if(army('disruptUntil')){atk.push({label:'虚实之策',factor:.85});def.push({label:'虚实之策',factor:.85});discipline.push({label:'虚实之策',factor:.85});}
 ...
 if(on('phalanx')||on('stun'))move.push({label:on('stun')?'眩晕':'方阵',factor:0});
