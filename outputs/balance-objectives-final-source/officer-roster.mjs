...
import {SKILL_ROUTES,commonRouteName} from './passives.mjs';
...
export function rosterMarkup({query='',sort='source',kind='all',page=0,selected=[]}={}){
 ...
 return `<p class="modal-intro">832 名武将库人物 · 3 名自建武将。所有武将均有五技能成长路线，41 位名将另有专属战法；可选择 1～6 人组成独立试炼阵容。</p>
...
export function officerDetailMarkup(id,scores={},types={}){
 ...
 ${SPECIAL_TACTICS[id]?`<h3 class="stats-section-title">专属战法 · ${esc(TACTICS_BOOK[SPECIAL_TACTICS[id]].name)}</h3><p>${esc(TACTICS_BOOK[SPECIAL_TACTICS[id]].description)}</p><p class="muted">战意 ${TACTICS_BOOK[SPECIAL_TACTICS[id]].threshold} · 冷却 ${TACTICS_BOOK[SPECIAL_TACTICS[id]].cooldown} 步 · 局部主动效果，战意达标后自动施放；占用一个战法槽，1 级可配置。</p>`:''}
