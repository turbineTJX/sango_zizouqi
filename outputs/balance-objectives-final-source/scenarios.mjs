...
import { newGame, makeOfficer, startBattle, configureBattleTerrain, chooseArmyAdvisor } from './engine.mjs';
import { recommendedTacticIds, roleTacticIds, setStatus, configureTactics, validLoadout, SPECIAL_TACTICS } from './tactics.mjs';
...
export function createScenario(id, seed, shieldPercent = 20, officerIds = null, customDraft = null) {
  ...
  if(config.ownTeam){
    [own,enemy].forEach((army,side)=>{
      ...
      army.units.forEach((u,i)=>{
        ...
        const role=custom&&u.type==='logistics'?'assault':u.type==='logistics'||i===entries.findIndex(v=>v.type==='spear'||v.type==='halberd')?'guard':u.intellect>u.force+10?'control':'assault';
  ...
  if(config.ownTeam){
    ...
    // setup is complete. Explicit enemy presets remain authoritative.
    planEnemyArmy(b);
