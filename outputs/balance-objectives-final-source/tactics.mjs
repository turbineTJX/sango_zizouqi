...
export function recommendedTacticIds(u) {
  ...
  // A cavalry exclusive is the payoff, not a substitute for approaching or
  // reaching the back line. Keep both tools and pay with the attack buff slot.
  if(special&&u.type==='cavalry')return [special,'gallop','rush'];
...
export function supportApproach(b,u) {
  if(u.type==='logistics'||hasStatus(b,u,'seal'))return null; // Auxiliaries keep fighting in melee.
  const skills=unitTactics(u),medical=skills.filter(s=>['screen','relay','bandage','regrowth','purify','supply','boarding','mist','nexus'].includes(s.effect)&&u.intent>=s.threshold&&(u.skillReady?.[s.id]||0)<=b.tick);
...
export function tacticTarget(b,u,s,range) {
  ...
  if(s.effect==='cleanse')return living(b,u.side).find(a=>distance(u,a)<=(s.range??range)&&NEGATIVE_STATUSES.some(key=>hasStatus(b,a,key)))||null;
