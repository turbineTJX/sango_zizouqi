...
test('combo enhances damage-over-time, intent reduction and control duration while respecting immunity',()=>{
 const x=scene();cast(x,x.own[0],'repeat');cast(x,x.own[1],'fire');assert.equal(x.target.statuses.burn.amount,Math.round((6+unitAttributes(x.own[1],x.b).martialPower*.03)*100/(100+unitAttributes(x.target,x.b).discipline)*1.25));assert.equal(x.target.statuses.burn.until,x.b.tick+8);
