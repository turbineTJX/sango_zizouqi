...
export function configureUnitTactics(state,unitId,ids) {
  ...
  if(live) {configureTactics(live,ids);live.skillReady={};live.tacticReady=0;live.tacticCasts={};live.cast=null;}
  return null;
}
...
export function stepBattle(b) {
  ...
  for (const side of sequence) for (const unit of [...activeUnits(b, side)]) {
    ...
    if (ready) {
      ...
      unit.tacticReady=b.tick+COMBAT.tacticGap;
  ...
  if (value.battle !== null) {
    ...
    b.sides.forEach((side, index) => {
      ...
      for (const u of side.units) {
        ...
        require(number(u.tacticReady,b.tick+COMBAT.tacticGap),'施法间隔数据无效');
