...
export const TACTICS_BOOK = {
  ...EXPANDED_TACTICS,
  smoke: scheme('惑阵',65,28,'confuse','shockwave','扰乱射程内敌军，使其混乱；时长随谋略威力增长、受军纪减免；混乱时随机转移阵位、无法攻击或施法',4),
  wildfire: scheme('火计',80,30,'wildfire','fire','对相邻最多 2 队造成 0.85 倍谋略伤害并灼烧 6 步；主目标已燃烧时命中伤害提高至 1.25 倍；灼烧每层为 6＋谋略威力×0.04，受军纪抵御；燃击 12 步，普攻续叠灼烧（最多 3 层、刷新 6 步）；自身谋攻 16 步，普攻改用 0.8 倍谋略威力、受军纪抵御',4),
  rally: scheme('振旅',25,22,'rally','banner','为 2 格内最多 2 队其他友军增加 24＋谋略威力×0.02 战意；仅在友军缺少至少 15 战意时发动',2),
  doubt: scheme('疑阵',55,26,'confuse','shockwave','攻击 2 格内敌军（1.6 倍谋略）并使其混乱；自身谋攻 10 步，普攻改用 0.8 倍谋略威力、受军纪抵御；需在前线承伤',2),
  ward: scheme('挑衅',35,24,'taunt','banner','嘲讽 5 格内一队敌军 4 步：优先攻击或合法接近自身，可吸引弓弩；受坚定保护、可净化，不越过有效 ZOC；不附带减伤',5),
  cleanse: scheme('解围',45,22,'cleanse','banner','清除 2 格内一队友军的控制与减益，给予兵力上限×(8%＋谋略威力/8000)护盾，持续 8 步；无减益不发动',2),
  lure: scheme('诱敌',55,24,'lure','charge','诱使 3 格内敌军向自身移动 1 格，并降低其防御 6 步；方阵不受诱导',3),
  harass: scheme('挫锐',30,22,'harass','banner','优先压制 3 格内威胁友军的敌人：降低 24＋谋略威力×0.03 战意，并使攻击 −20% 持续 6 步；不造成伤害',3),
  relay: scheme('策应',35,26,'relay','banner','为 2 格内最多 2 队其他友军恢复兵力上限×(7%＋谋略威力/10000)的伤兵，冷却缩短 2 步，并加速 4 步；不治疗自身，不复活；施放后重置普攻间隔',2),
  ambush: scheme('伏弩',25,18,'ambush','shockwave','伏击 2 格内敌人（0.6 倍谋略），使其移速减半、攻击 −20%，持续 6 步；优先尚未被削弱的敌人',2),
  seal: scheme('封喉',65,26,'seal','slash','封锁 4 格内敌人的后续战法；基础 5 步，受军纪减免；优先战意高者，不造成伤害',4),
  screen: scheme('救护',35,24,'screen','banner','为 3 格内一队受损友军恢复兵力上限×(8%＋谋略威力/10000)的伤兵，附加 4% 护盾 6 步；无伤兵时仅保护接敌者，不复活；施放后重置普攻间隔',3),
  fire: tactic('火矢',40,22,'fire','fire','射击（1.0 倍武技）并灼烧 6 步；自身燃击 10 步，普攻续叠灼烧，最多 3 层、刷新 6 步；每层为 6＋武技威力×0.03，受军纪抵御、按施放时兵力衰减'),
  scatter: tactic('箭雨',70,26,'scatter','volley','攻击射程内最多 3 支相邻敌军，单队伤害较低'),
  suppress: tactic('阻射',25,18,'suppress','volley','轻射（0.7 倍武技）并使目标移动减慢 6 步；优先未迟滞且靠近友军的敌人'),
  thrust: tactic('贯阵',25,18,'thrust','slash','刺击相邻目标（1.3 倍武技），波及其身后一格敌军（1.1 倍）；优先可贯穿的阵列'),
  phalanx: tactic('枪阵',65,26,'phalanx','banner','接敌时列阵 8 步：自身减伤 30%，停止移动并免疫击退'),
  strike: tactic('奋击',75,22,'strike','slash','攻击相邻敌军（1.7 倍武技）；目标被控制、迟滞或破防时提高至 2.5 倍，优先符合条件者'),
  gallop: tactic('疾驰',0,22,'gallop','charge','无需战意；行军接敌或 2 格内有敌人时发动：自身减伤 25% 持续 6 步；有接敌路线时额外移速 +1；不造成伤害'),
  rush: tactic('冲阵',60,24,'rush','charge','沿最多 3 格空闲路线冲击（1.6 倍武技，对弓弩 2.2 倍），优先可达后排；骑兵突入后追击 8 步，对弓弩普攻 +35%；不能穿过有效 ZOC'),
  valor: tactic('奋战',85,26,'valor','banner','交战范围内，自身攻击提高 25%，同时自身破防（防御 −20%），持续 8 步；适合侧翼持续输出'),
  repeat: tactic('连珠',40,22,'repeat','volley','对同一敌军连续射击两次'),
  retreatShot: tactic('退射',60,20,'retreatShot','volley','近敌威胁时退至更安全的相邻空格，再射击'),
  pierce: tactic('破甲',25,22,'pierce','slash','轻射（0.7 倍武技）并降低目标防御 20%，持续 8 步；优先尚未破防的高防御敌军'),
...
for(const [id,definition] of Object.entries(TACTICS_BOOK)){
  const s=TACTICS_BOOK[id]={...definition,cooldown:Math.ceil(definition.cooldown*COMBAT.tacticCooldownScale)};
  s.id=id;s.category ||= 'force';s.terrainDescription=terrainTacticDescription(s);
}
...
const TOOL_NOTES = {
  ...
  doubt:['近程谋攻','需靠近前线；控制受军纪和控制保护限制'],ward:['远距引战','只影响一队，吸引火力但不减伤；需队友护盾或枪阵承伤'],cleanse:['解除压制','无减益时不发动，无法预先套盾'],
  ...
  fire:['叠燃消耗','需连续攻击同一目标；停火、换目标或被净化会损失积累'],scatter:['范围覆盖','单体倍率低；敌军分散时收益低'],suppress:['阻击减速','命中伤害低，贴身肉搏时减速价值下降'],
  smoke:['远程混乱','无直接伤害；控制保护期间无法重复控制'],wildfire:['谋攻叠燃','门槛高、冷却长；持续射击才能叠满灼烧，净化清空层数'],rally:['启动友军战法','不鼓舞自身，队友满战意时无收益'],
  repeat:['单体爆发','不破甲、不控制，也无法自保'],retreatShot:['撤步自保','需要安全空格，会离开原有支援阵位'],pierce:['集火破甲','自身命中伤害低，需要物理队友跟进'],
...
export const TACTIC_ROLES = {...EXPANDED_ROLES,
  spear:[{id:'assault',name:'物理战坦',ids:['phalanx','strike','thrust'],position:'前排承伤，配合破防队友输出；可将奋击换挑衅偏控制',cost:'放弃嘲讽、谋略伤害与净化'},
    {id:'guard',name:'纯坦克',ids:['cleanse','phalanx','ward'],position:'站在主力前方，枪阵承伤、挑衅吸引火力',cost:'放弃全部直接伤害战法与混乱控制'},
...
export function setStatus(b,u,key,duration,extra={}) {
  ...
  if(key==='burn') {
    ...
    u.statuses.burn={until:Math.max(current?.until||0,b.tick+duration+1),sourceId,baseAmount,stacks,amount:baseAmount*stacks};return;
  }
  if(key==='curse'){const stacks=Math.min(3,(hasStatus(b,u,'curse')?u.statuses.curse.stacks:0)+1);u.statuses.curse={until:b.tick+duration+1,stacks};return;}
...
export function readyTactic(b,u,range) {
  if(hasStatus(b,u,'seal') || u.tacticReady>b.tick)return null;
