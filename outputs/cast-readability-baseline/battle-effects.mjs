...
const damageLabel = e => `−${e.damage}${e.intentBlock?' · '+e.intentBlock:''}`;
...
export class BattleEffects {
  ...
  update(battle, { paused, speed, mode = 'clear' }) {
    ...
    for (const events of casts.values()) {
      this.notice(events);
      if(paused){this.showResult(events);continue;}
  ...
  showResult(events) {
    ...
    this.result.textContent=`${e.side?'敌军':'我军'} · ${e.name}「${e.label}」\n${outcomeLines(events).join(' / ')}`;
    this.result.title=this.result.textContent+'\n点击暂停，查看完整记录';
  }
  notice(events) {
    const e=events[0];
    ...
    item.append(badge, name, title, detail); this.feed.append(item);
  ...
  draw(time) {
    ...
    for (const e of this.isCinematicPlaying() || (clear && this.paused) ? [] : clear ? this.clearEffects() : this.items) {
      ...
      if (clear) this.drawClear(e,p,from,to,cell);
        ...
        if (p > .2 && (e.damage || e.text)) this.label(e.text || damageLabel(e), to.x, to.y - cell * (.2 + p * .4), e.comboLevel?'#ffe38a':'#fff1b4', Math.min(1, (1 - p) * 2), clamp(cell * (e.comboLevel?.52:.43), 14, e.comboLevel?30:26));
      }
      c.restore();
    }
