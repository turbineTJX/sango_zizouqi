...
export const COMMON_ROUTES = {
 halberd:['iron','shelter','joint','desperate','fortress'],logistics:['scholar','discipline','shield','calm','aid'],siege:['assault','scholar','joint','steady','wisdom'],ship:['iron','discipline','spirit','shield','fortress'],
...
export function commonRouteKey(u) {
  ...
  if(source.politics>=80&&source.politics>=source.intellect&&source.politics>=source.force+15)return 'support';
  if(source.intellect>=65&&source.intellect>=source.force+15)return 'strategist';
  return source.type;
}
export const commonRouteName=u=>({halberd:'戟阵修习',logistics:'辎重修习',siege:'机巧修习',ship:'水战修习',spear:'步阵修习',cavalry:'骑战修习',archer:'弓术修习',crossbow:'弩术修习',strategist:'谋攻修习',support:'辅军修习'}[commonRouteKey(u)]);
...
function personalCondition(b,u,target,p){
  ...
  switch(p.trigger){
    ...
    case 'debuffed':return !!b&&['stun','confuse','seal','slow','armorBreak','weaken','burn','scorch'].some(k=>(target?.statuses?.[k]?.until||0)>b.tick);
    default:return false;
  }
}
export const officerLevel = u => u.level ?? 1;
export const skillRoute = u => SKILL_ROUTES[u.id] || COMMON_ROUTES[commonRouteKey(u)] || COMMON_ROUTES.spear;
...
export function passiveDamageMultiplier(b,u,target,kind,basic=kind==='basic') {
  let bonus=0;
  const personal=personalPassive(u);
  if(target.type!=='gate'&&(personal?.kind===kind||basic&&personal?.kind==='basic')&&personalCondition(b,u,target,personal))bonus+=personal.bonus;
  if(basic){
