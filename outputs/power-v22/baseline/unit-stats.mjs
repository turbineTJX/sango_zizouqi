...
import {troopStrength} from './troop-capacity.mjs';
...
export function unitAttributes(u,b=null) {
 ...
 if(on('curse')){const m={label:'衰咒 '+u.statuses.curse.stacks+'层',factor:1-u.statuses.curse.stacks*.06};atk.push(m);strategy.push(m);discipline.push(m);}
 if(on('bulwark')){def.push({label:'铁壁',factor:1.3});discipline.push({label:'铁壁',factor:1.2});move.push({label:'铁壁',factor:.5});}
 if(on('camp'))def.push({label:'营垒',factor:1.25});
 if(on('nexus')){strategy.push({label:'阵枢',factor:1.2});discipline.push({label:'阵枢',factor:1.2});}
 if(on('shaken'))strategy.push({label:'震军',factor:.85});
 if(on('emplaced')){atk.push({label:'架设',factor:1.2});range.push({label:'架设',add:1});}
 ...
 if(on('valor'))atk.push({label:'奋战',factor:1.25});
 ...
 stat('attackSpeed',1000/(700*t.interval),0,'兵种攻击间隔 '+t.interval+' 步');out.attackInterval=t.interval/(passives.attackSpeed?.[0]?.factor||1)*(on('shaken')?1.25:1);out.minRange=t.minRange||0;
 ...
 out.damageReduction=1-(on('phalanx')?.7:1)*(on('ward')?1-u.statuses.ward.percent/100:1)*(on('anchored')?.75:1);
 out.strength=troopStrength(u.hp??u.troops);
 return out;
}
